...
def CalculateMomentWithML(input_list):
    ...
    for i in range(len(input_list)):
        normalised_input.append((Y_max[i]-Y_min[i])*(input_list[i]-X_min[i]) / (X_max[i] - X_min[i]) + Y_min[i])

    input_array = np.array(normalised_input)

    IW1 = np.array([[0.00572662427139449,0.765475186245782,-0.376581821328129,-0.0356168354299637,0.510951913724441,-0.286732023674033],
...
# --- Streamlit UI ---
st.title("Simply supported I-section — Calculator")
...
if st.button("Calculate"):
    ...
    st.write(f"**Predicted Moment Capacity** = {moment:.2f} kN·mm")
    st.write(f"**Weight** = {weight:.4f} kg/m")
